...
public class FasterFallingEnchant extends ZanyEnchant {
	 ...
	 */
	public FasterFallingEnchant(TextComponent display, int anvilCost, int maxLevel, int weight, int minCost, int maxCost, int increasePerLevel, EquipmentSlotGroup[] activeSlots, TagKey<ItemType>[] supportedItemTags, TagKey<Enchantment>[] enchantmentTags) {
	...
	public void onMove(PlayerMoveEvent event, Location nLoc) {
		Location oLoc = event.getFrom();
		Material[] nonGroundBlocks = new Material[]{Material.WATER, Material.LAVA, Material.AIR, Material.CAVE_AIR, Material.VOID_AIR};
		boolean falling = oLoc.getY() < nLoc.getY();
		Block nextBlock = event.getTo().getWorld().getBlockAt(nLoc.subtract(0.0, 1.0, 0.0));
		boolean nextBlockGround = Arrays.stream(nonGroundBlocks).toList().contains(nextBlock.getType());
		if (falling) {
			if (!nextBlockGround) {
				event.getPlayer().teleport(event.getPlayer().getLocation().subtract(0.0, 0.5, 0.0));
			}
