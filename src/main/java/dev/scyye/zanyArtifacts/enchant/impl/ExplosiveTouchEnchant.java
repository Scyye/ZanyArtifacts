...
public class ExplosiveTouchEnchant extends ZanyEnchant {
	 ...
	 */
	public ExplosiveTouchEnchant(TextComponent display, int anvilCost, int maxLevel, int weight, int minCost, int maxCost, int increasePerLevel, EquipmentSlotGroup[] activeSlots, TagKey<ItemType>[] supportedItemTags, TagKey<Enchantment>[] enchantmentTags) {
	...
	public void onBreakBlock(Block block, ItemStack item, BlockBreakEvent event) {
		...
		location.getWorld().createExplosion(location, 2.0F*(this.getLevel(item)), false, false);
	}

	@Override
	public void onMove(PlayerMoveEvent var1, Location var2) {
